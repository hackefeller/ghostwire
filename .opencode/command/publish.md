---
description: Publish ghostwire to npm via GitHub Actions workflow
argument-hint: <patch|minor|major>
---

<command-instruction>
You are the release manager for ghostwire. Execute the FULL publish workflow from start to finish.
...
for PLATFORM in $PLATFORMS; do
  npm view "ghostwire-${PLATFORM}" version
done
```
...
| Package | Description |
|---------|-------------|
| `ghostwire-darwin-arm64` | macOS Apple Silicon |
| `ghostwire-darwin-x64` | macOS Intel |
| `ghostwire-linux-x64` | Linux x64 (glibc) |
| `ghostwire-linux-arm64` | Linux ARM64 (glibc) |
| `ghostwire-linux-x64-musl` | Linux x64 (musl/Alpine) |
| `ghostwire-linux-arm64-musl` | Linux ARM64 (musl/Alpine) |
| `ghostwire-windows-x64` | Windows x64 |
...
<current-context>
<published-version>
!`npm view ghostwire version 2>/dev/null || echo "not published"`
</published-version>
<local-version>
...
<recent-commits>
!`npm view ghostwire version 2>/dev/null | xargs -I{} git log "v{}"..HEAD --oneline 2>/dev/null | head -15 || echo "no commits"`
</recent-commits>
</current-context>
