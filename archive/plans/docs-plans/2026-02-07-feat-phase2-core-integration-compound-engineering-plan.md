...
**Namespace Consistency**: All components use `compound:` prefix
- Agents: `compound:kieran-rails-reviewer`, `compound:dhh-rails-reviewer`
- Commands: `compound:workflows:plan`, `compound:code:refactor`  
- Skills: `compound:frontend-design`, `compound:andrew-kane-gem-writer`
...
Based on `src/agents/oracle.ts` pattern:
...
// Agent metadata for Sisyphus integration
...
const COMPOUND_COMMAND_DEFINITIONS: Record<string, Omit<CommandDefinition, "name">> = {
  "compound:workflows:plan": workflowsPlanCommand,
  "compound:workflows:review": workflowsReviewCommand,
  "compound:code:refactor": codeRefactorCommand,
  "compound:git:smart-commit": gitSmartCommitCommand,
...
export const BuiltinAgentNameSchema = z.enum([
  ...
  "compound:kieran-rails-reviewer", "compound:kieran-python-reviewer", "compound:kieran-typescript-reviewer",
  "compound:dhh-rails-reviewer", "compound:code-simplicity-reviewer",
  // Compound Research Agents  
  "compound:framework-docs-researcher", "compound:learnings-researcher", 
  "compound:best-practices-researcher", "compound:git-history-analyzer",
  // ... all 28 compound agents
])

// Add compound commands to schema
export const BuiltinCommandNameSchema = z.enum([
  "init-deep", "start-work",
  // Compound Commands
  "compound:workflows:plan", "compound:workflows:review", "compound:code:refactor",
  // ... all 24 compound commands  
])
...
- All agents discoverable in Sisyphus prompt building
...
- All commands accessible via /compound: prefix
...
- [ ] Namespace isolation with `compound:` prefix for all components
...
- Use `compound:` namespace isolation consistently  
...
- **Agent Patterns**: src/agents/oracle.ts:1, src/agents/sisyphus.ts:1
...
- **Test Patterns**: src/agents/momus.test.ts:1
...
- [ ] Implement agent metadata for Sisyphus integration
