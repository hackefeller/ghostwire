 ...
 * @returns {string} Package name like "ghostwire-darwin-arm64"
 ...
 * @returns {string} Relative path like "ghostwire-darwin-arm64/bin/ghostwire"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/ghostwire${ext}`;
}
