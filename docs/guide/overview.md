...
Learn about Ruach, a plugin that transforms OpenCode into the best agent harness.
...
Ruach automatically configures models based on your available providers. You don't need to manually specify every model.

### How Models Are Determined

**1. At Installation Time (Interactive Installer)**

When you run `bunx ruach install`, the installer asks which providers you have:
...
- Z.ai Coding Plan?
 ...
 Based on your answers, it generates `~/.config/opencode/ruach.json` with optimal model assignments for each agent and category.
