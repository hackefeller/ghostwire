...
- Preserve `ghostwire` feature depth while adding a clean ingestion layer for Claude Code plugins.
...
1. **Core Orchestration**: ghostwire agents, hooks, tools, MCPs, shared utilities.
...
### 1) Core Orchestration (Existing ghostwire)

**Location**: `src/agents`, `src/hooks`, `src/tools`, `src/mcp`, `src/features`, `src/shared`

Responsibilities:

- Route tasks through orchestration hooks (Nexus Orchestrator, Cipher Operator, etc.).
...
| P0 | `performance-seer-advisor` | `agent:performance-seer-advisor` | Perf review baseline |
...
Test coverage should follow the ghostwire TDD requirements.
