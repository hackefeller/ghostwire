...
Ruach provides a core set of 10 specialized agents plus additional compound agents under the `compound:` namespace. Each has distinct expertise, optimized models, and tool permissions.
...
Customize agent models, prompts, and permissions in `ruach.json`. See [Configuration](configurations.md#agents).
...
Ruach provides two browser automation providers, configurable via `browser_automation_engine.provider`:
