...
console.log("=== Publishing ruach (multi-package) ===\n")
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/code-yeongyu/ruach/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/ruach.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
