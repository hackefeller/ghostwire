...
describe("mcp-oauth check", () => {
  ...
  describe("checkMcpOAuthTokens", () => {
    ...
    it("returns warn when some tokens expired", async () => {
      ...
      expect(result.details?.some((d: string) => d.includes("Expired"))).toBe(true);
    });
    ...
    it("includes token details in output", async () => {
      ...
      expect(result.details?.some((d: string) => d.includes("api.example.com"))).toBe(true);
      expect(result.details?.some((d: string) => d.includes("auth.example.com"))).toBe(true);
    });
  });
});
