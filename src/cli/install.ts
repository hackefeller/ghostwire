...
async function runNonTuiInstall(args: InstallArgs): Promise<number> {
  ...
  if (!validation.valid) {
    ...
    console.log()
    printInfo("Usage: bunx ruach install --no-tui --claude=<no|yes|max20> --gemini=<no|yes> --copilot=<no|yes>")
    console.log()
    return 1
  }
  ...
  printStep(step++, totalSteps, "Adding ruach plugin...")
  ...
  printStep(step++, totalSteps, "Writing ruach configuration...")
  ...
  console.log(`  ${color.dim("gh api --silent --method PUT /user/starred/code-yeongyu/ruach >/dev/null 2>&1 || true")}`)
  console.log()
...
export async function install(args: InstallArgs): Promise<number> {
  ...
  s.start("Adding ruach to OpenCode config")
  const pluginResult = await addPluginToOpenCodeConfig(VERSION)
  if (!pluginResult.success) {
  ...
  s.start("Writing ruach configuration")
  const omoResult = writeOmoConfig(config)
  if (!omoResult.success) {
  ...
  p.log.message(`  ${color.dim("gh api --silent --method PUT /user/starred/code-yeongyu/ruach >/dev/null 2>&1 || true")}`)
