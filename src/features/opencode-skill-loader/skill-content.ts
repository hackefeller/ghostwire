...
export function injectGitMasterConfig(template: string, config?: GitMasterConfig): string {
	...
	if (commitFooter) {
		...
		sections.push("```")
		sections.push(`Ultraworked with [Sisyphus](https://github.com/code-yeongyu/ruach)`) 
		sections.push("```")
		sections.push(``)
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(`git commit -m "{Commit Message}" -m "Ultraworked with [Sisyphus](https://github.com/code-yeongyu/ruach)" -m "Co-authored-by: Sisyphus <clio-agent@sisyphuslabs.ai>"`)
		sections.push("```")
	} else if (commitFooter) {
		sections.push(`**Example:**`)
		sections.push("```bash")
		sections.push(`git commit -m "{Commit Message}" -m "Ultraworked with [Sisyphus](https://github.com/code-yeongyu/ruach)"`)
		sections.push("```")
