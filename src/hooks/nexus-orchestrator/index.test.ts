...
describe("nexus-orchestrator hook", () => {
   const TEST_DIR = join(tmpdir(), "nexus-orchestrator-test-" + Date.now())
  ...
  describe("tool.execute.after handler", () => {
    ...
    test("should ignore non-delegate_task tools", async () => {
     ...
     test("should append standalone verification when no boulder state but caller is Nexus Orchestrator", async () => {
       ...
       setupMessageStorage(sessionID, "nexus-orchestrator")
    ...
    describe("Write/Edit tool direct work reminder", () => {
      const ORCHESTRATOR_SESSION = "orchestrator-write-test"
       ...
       beforeEach(() => {
         setupMessageStorage(ORCHESTRATOR_SESSION, "nexus-orchestrator")
       })
      ...
      test("should NOT append reminder when non-orchestrator writes outside .ghostwire/", async () => {
        ...
        setupMessageStorage(nonOrchestratorSession, "cipher-runner")
  ...
  describe("session.idle handler (boulder continuation)", () => {
    const MAIN_SESSION_ID = "main-session-123"
     ...
     beforeEach(() => {
       ...
       setupMessageStorage(MAIN_SESSION_ID, "nexus-orchestrator")
     })
    ...
    test("should include plan progress in continuation prompt", async () => {
     ...
     test("should not inject when last agent is not Nexus Orchestrator", async () => {
       ...
       cleanupMessageStorage(MAIN_SESSION_ID)
       setupMessageStorage(MAIN_SESSION_ID, "cipher-operator")
