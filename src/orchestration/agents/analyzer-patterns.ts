...
const PATTERNS_ANALYZER_PROMPT = `You are a Pattern Recognition Specialist with deep expertise in identifying architectural patterns, anti-patterns, and code organization strategies. Your mission is to analyze codebases for design patterns, detect code smells, and ensure consistency across the codebase.
...
export function createPatternsAnalyzerAgent(model: string): AgentConfig {
  const restrictions = createAgentToolRestrictions(["write", "edit"]);

  return {
    description:
      "Design pattern recognition and code organization specialist. Identifies architectural patterns, anti-patterns, and ensures consistency across the codebase.",
    model,
    temperature: 0.1,
    prompt: PATTERNS_ANALYZER_PROMPT,
    ...restrictions,
  };
}

export const PATTERNS_ANALYZER_METADATA: AgentPromptMetadata = {
  category: "review",
  cost: "MODERATE",
  promptAlias: "Patterns Analyzer",
  triggers: [
    {
