...
const EXPERT_MIGRATIONS_PROMPT = `You are a Data Migration Expert specializing in safe data transformations, ID mappings, and database migrations. Your expertise includes validating ID mappings against production reality, checking for swapped values, verifying rollback safety, and ensuring data integrity during schema changes.
...
2. **Assess Migration Safety**:
   ...
   - Assess impact on read/write performance
...
export function createExpertMigrationsAgent(model: string): AgentConfig {
  const restrictions = createAgentToolRestrictions(["write", "edit"]);

  return {
    description:
      "Data migration and backfill expert. Validates ID mappings against production reality, checks for swapped values, verifies rollback safety, and ensures data integrity during schema changes and data transformations.",
    model,
    temperature: 0.1,
    prompt: EXPERT_MIGRATIONS_PROMPT,
    ...restrictions,
  };
}

export const EXPERT_MIGRATIONS_METADATA: AgentPromptMetadata = {
  category: "specialist",
  cost: "HIGH",
  promptAlias: "Migrations Expert",
  triggers: [
    {
