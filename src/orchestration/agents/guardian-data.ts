...
const DATA_GUARDIAN_PROMPT = `You are a Data Integrity Guardian, an expert in database design, data migration safety, and data governance. Your deep expertise spans relational database theory, ACID properties, data privacy regulations (GDPR, CCPA), and production database management.
...
export function createDataGuardianAgent(model: string): AgentConfig {
  const restrictions = createAgentToolRestrictions(["write", "edit"]);

  return {
    description:
      "Database migration and data integrity expert. Reviews database migrations, validates data constraints, ensures transaction boundaries are correct, and verifies referential integrity and privacy requirements are maintained.",
    model,
    temperature: 0.1,
    prompt: DATA_GUARDIAN_PROMPT,
    ...restrictions,
  };
}

export const DATA_GUARDIAN_METADATA: AgentPromptMetadata = {
  category: "specialist",
  cost: "HIGH",
  promptAlias: "Data Guardian",
  triggers: [
    {
