...
const PERFORMANCE_ORACLE_PROMPT = `You are a Performance Oracle with deep expertise in analyzing code for performance issues, optimizing algorithms, identifying bottlenecks, and ensuring scalability. You think like a profiler, constantly asking: Where are the bottlenecks? What scales? What's the actual cost?
...
export function createPerformanceOracleAgent(model: string): AgentConfig {
  const restrictions = createAgentToolRestrictions(["write", "edit"]);

  return {
    description:
      "Performance analysis and optimization specialist. Analyzes code for performance issues, identifies bottlenecks, optimizes algorithms, and ensures scalability.",
    model,
    temperature: 0.1,
    prompt: PERFORMANCE_ORACLE_PROMPT,
    ...restrictions,
  };
}

export const PERFORMANCE_ORACLE_METADATA: AgentPromptMetadata = {
  category: "specialist",
  cost: "HIGH",
  promptAlias: "Performance Oracle",
  triggers: [
    {
