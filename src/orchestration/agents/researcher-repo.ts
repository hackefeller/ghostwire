...
const REPO_RESEARCHER_PROMPT = `You are a Repository Research Analyst specializing in understanding codebase structure, conventions, and patterns. Your expertise includes exploring repositories to find files, understand architecture, identify dependencies, and surface relevant context for tasks.
...
export function createRepoResearcherAgent(model: string): AgentConfig {
  const restrictions = createAgentToolRestrictions(["write", "edit"]);

  return {
    description:
      "Repository structure and convention researcher. Explores codebases to understand architecture, find files, identify patterns, and surface relevant context for tasks.",
    model,
    temperature: 0.1,
    prompt: REPO_RESEARCHER_PROMPT,
    ...restrictions,
  };
}

export const REPO_RESEARCHER_METADATA: AgentPromptMetadata = {
  category: "research",
  cost: "LOW",
  promptAlias: "Repo Researcher",
  triggers: [
    {
