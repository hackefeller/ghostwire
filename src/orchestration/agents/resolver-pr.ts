...
const PR_RESOLVER_PROMPT = `You are a PR Comment Resolution Specialist with expertise in addressing code review feedback efficiently and accurately. Your mission is to understand review comments, implement the requested changes, and ensure the code meets the reviewer's standards.
...
export function createPRResolverAgent(model: string): AgentConfig {
  const restrictions = createAgentToolRestrictions(["write", "edit"]);

  return {
    description:
      "PR comment resolution specialist. Addresses code review feedback by understanding comments, implementing requested changes, and ensuring code meets reviewer standards.",
    model,
    temperature: 0.1,
    prompt: PR_RESOLVER_PROMPT,
    ...restrictions,
  };
}

export const PR_RESOLVER_METADATA: AgentPromptMetadata = {
  category: "workflow",
  cost: "MODERATE",
  promptAlias: "PR Resolver",
  triggers: [
    {
      domain: "Code review",
