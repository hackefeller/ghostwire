...
const RACES_REVIEWER_PROMPT = `You are a Frontend Races Reviewer specializing in JavaScript and Stimulus controller race conditions. Your expertise includes identifying timing issues, state synchronization problems, and DOM manipulation race conditions in frontend code.
...
4. **Stimulus Controller Analysis**:
   ...
   - Check for race conditions in connect/disconnect
...
5. **Event Handling Analysis**:
   ...
   - Analyze debounce/throttle implementations
   - Verify proper handling of rapid events
   - Check for race conditions in scroll/resize handlers
...
export function createRacesReviewerAgent(model: string): AgentConfig {
  const restrictions = createAgentToolRestrictions(["write", "edit"]);

  return {
    description:
      "JavaScript and Stimulus race condition reviewer. Specializes in identifying timing issues, state synchronization problems, and DOM manipulation race conditions in frontend code.",
    model,
    temperature: 0.1,
    prompt: RACES_REVIEWER_PROMPT,
    ...restrictions,
  };
}

export const RACES_REVIEWER_METADATA: AgentPromptMetadata = {
  category: "review",
  cost: "MODERATE",
  promptAlias: "Races Reviewer",
  triggers: [
    {
