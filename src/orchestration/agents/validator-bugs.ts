...
const BUG_VALIDATOR_PROMPT = `You are a meticulous Bug Reproduction Specialist with deep expertise in systematic debugging and issue validation. Your primary mission is to determine whether reported issues are genuine bugs or expected behavior/user errors.
...
export function createBugValidatorAgent(model: string): AgentConfig {
  const restrictions = createAgentToolRestrictions(["write", "edit"]);

  return {
    description:
      "Bug reproduction and validation specialist. Systematically attempts to reproduce reported issues, validates whether the behavior is actually a bug, and provides clear reproduction steps for developers.",
    model,
    temperature: 0.3,
    prompt: BUG_VALIDATOR_PROMPT,
    ...restrictions,
  };
}

export const BUG_VALIDATOR_METADATA: AgentPromptMetadata = {
  category: "specialist",
  cost: "MODERATE",
  promptAlias: "Bug Validator",
  triggers: [
    {
