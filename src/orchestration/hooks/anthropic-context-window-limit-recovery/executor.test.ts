...
describe("executeCompact lock management", () => {
  ...
  test("clears lock when summarize throws exception", async () => {
    ...
    mockClient.session.summarize = mock(() => Promise.reject(new Error("Network timeout")));
    autoCompactState.errorDataBySession.set(sessionID, {
      errorType: "token_limit",
      currentTokens: 100000,
      maxTokens: 200000,
  ...
  test("clears lock when truncation is sufficient", async () => {
    ...
    await executeCompact(sessionID, msg, autoCompactState, mockClient, directory, experimental);
  ...
  test("clears lock when client.tui.showToast throws", async () => {
    ...
    mockClient.tui.showToast = mock(() => Promise.reject(new Error("Toast failed")));
    autoCompactState.errorDataBySession.set(sessionID, {
      errorType: "token_limit",
      currentTokens: 100000,
      maxTokens: 200000,
  ...
  test("clears lock when prompt_async in continuation throws", async () => {
    ...
    mockClient.session.prompt_async = mock(() => Promise.reject(new Error("Prompt failed")));
    autoCompactState.errorDataBySession.set(sessionID, {
      errorType: "token_limit",
      currentTokens: 100000,
      maxTokens: 200000,
