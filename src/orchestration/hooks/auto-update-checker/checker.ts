...
export function updatePinnedVersion(
  ...
  try {
    ...
    if (!regex.test(pluginArrayContent)) {
      log(
        `[grid-auto-update-checker] Entry "${oldEntry}" not found in plugin array of ${configPath}`,
      );
      return false;
    }
...
export async function checkForUpdate(directory: string): Promise<UpdateCheckResult> {
  ...
  log(
    `[grid-auto-update-checker] Current: ${currentVersion}, Latest (${channel}): ${latestVersion}, NeedsUpdate: ${needsUpdate}`,
  );
  return {
    needsUpdate,
    currentVersion,
