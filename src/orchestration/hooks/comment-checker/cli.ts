...
function debugLog(...args: unknown[]) {
  if (DEBUG) {
    const msg = `[${new Date().toISOString()}] [grid-comment-checker:cli] ${args.map((a) => (typeof a === "object" ? JSON.stringify(a, null, 2) : String(a))).join(" ")}\n`;
...
/**
 * Asynchronously get grid-comment-checker binary path.
...
/**
 * Synchronously get grid-comment-checker path (no download).
...
/**
 * Run grid-comment-checker CLI with given input.
...
export async function runCommentChecker(
  ...
  if (!binaryPath) {
    debugLog("grid-comment-checker binary not found");
    return { hasComments: false, message: "" };
  }

  if (!existsSync(binaryPath)) {
    debugLog("grid-comment-checker binary does not exist:", binaryPath);
    return { hasComments: false, message: "" };
  }
  ...
  try {
    ...
    debugLog("failed to run grid-comment-checker:", err);
    return { hasComments: false, message: "" };
  }
}
