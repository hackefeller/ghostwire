...
import {
  COMPOUND_SKILLS,
  COMPOUND_SKILL_NAMES,
} from "../src/execution/features/builtin-skills/compound-skills";
...
describe("Compound Engineering Skills", () => {
  describe("Skill Definitions", () => {
    ...
    test("all skill names use grid: prefix", () => {
      //#given
      const invalidSkills = COMPOUND_SKILLS.filter(
        (skill) => !skill.name.startsWith("grid:"),
      );
    ...
    test("all skills have descriptions", () => {
      //#given
      const skillsWithoutDesc = COMPOUND_SKILLS.filter(
        (skill) => !skill.description || skill.description.length === 0,
      );
    ...
    test("all skills have templates", () => {
      //#given
      const skillsWithoutTemplate = COMPOUND_SKILLS.filter(
        (skill) => !skill.template || skill.template.length === 0,
      );
    ...
    test("skill descriptions are informative (min 20 chars)", () => {
      //#given
      const shortDescriptions = COMPOUND_SKILLS.filter(
        (skill) => skill.description.length < 20,
      );
    ...
    test("skill templates contain meaningful content", () => {
      //#given
      const thinTemplates = COMPOUND_SKILLS.filter(
        (skill) => skill.template.length < 50,
      );
  ...
  describe("Skill Names Export", () => {
    ...
    test("all skill names in export are valid", () => {
      ...
      //#when
      const invalidNames = COMPOUND_SKILL_NAMES.filter(
        (name) => !skillNameSet.has(name),
      );
  ...
  describe("Skill Interface Compliance", () => {
    ...
    test("no skills have placeholder content", () => {
      ...
      const placeholderSkills = COMPOUND_SKILLS.filter((skill) =>
        ["TODO", "PLACEHOLDER", "FIXME", "XXX", "IMPLEMENT ME"].some(
          (marker) =>
            skill.description.toUpperCase().includes(marker) ||
            skill.template.toUpperCase().includes(marker),
        ),
      );
